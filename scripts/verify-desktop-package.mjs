...
const allowed =
  /^\/(main\.cjs|preload\.cjs|package\.json|BUILD-INFO\.json|THIRD-PARTY-NOTICES\.txt|사용안내\.md|renderer(?:\/.*)?|migrations(?:\/\d[^/]*\.sql)?)$/;
for (const path of entries) {
  assert(allowed.test(path), `Unexpected packaged file: ${path}`);
  assert(
    !/\.(sqlite|db|map)$|\/\.env|node_modules|ai-settings|smoke-result/i.test(
      path,
    ),
    `Private or development file packaged: ${path}`,
  );
}
for (const path of [
  "main.cjs",
  ...
  assert(extractFile(asar, path).length > 0);
...
assert.equal(
  executable.readUInt16LE(peOffset + 4),
  0x8664,
  "App must be Windows x64",
);
...
assert.equal(
  version,
  JSON.parse(await readFile("package.json", "utf8")).desktopVersion,
);
const installerName = `LinkSpring-Setup-${version}-x64.exe`;
const installer = await readFile(`release/${installerName}`);
assert.equal(installer.toString("ascii", 0, 2), "MZ");
assert(
  installer.length > 50_000_000,
  "Full offline installer should include Electron",
);
...
const result = {
  ...
  packageAllowlist: "passed",
  windowsExecutionTest: "not performed on this macOS host",
};
