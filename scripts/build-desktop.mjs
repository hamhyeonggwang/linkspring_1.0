...
for (const name of await readdir("drizzle"))
  if (name.endsWith(".sql"))
    await copyFile(`drizzle/${name}`, `desktop-build/migrations/${name}`);
const root = JSON.parse(await readFile("package.json", "utf8"));
await writeFile(
  "desktop-build/package.json",
  JSON.stringify(
    {
      name: "linkspring-desktop",
      version: root.desktopVersion,
      main: "main.cjs",
      description:
        "이어:봄 — 비영리 서비스의 빈 일정과 대기자를 연결하는 로컬 업무 도구",
      author: "LinkSpring",
      private: true,
      license: "UNLICENSED",
    },
...
await writeFile(
  "desktop-build/BUILD-INFO.json",
  JSON.stringify(
    {
      version: root.desktopVersion,
      electron: root.devDependencies.electron,
...
const modules = [
  ...
  ...JSON.parse(await readFile("desktop-build/renderer-modules.json", "utf8")),
];
const roots = new Set();
for (const input of modules) {
  ...
  roots.add(
    normalized.slice(0, index + marker.length) +
      parts.slice(0, parts[0].startsWith("@") ? 2 : 1).join("/"),
  );
}
let notices =
  "Third-party notices for libraries included in LinkSpring. Electron and Chromium notices are also provided in the installation directory.\n";
  ...
  if (!files.length) {
    if (pkg.name === "react-remove-scroll-bar")
      notices += await readFile(
        "vendor/react-remove-scroll-bar.LICENSE",
        "utf8",
      );
